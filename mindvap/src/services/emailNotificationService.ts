...
import EmailTemplateService, { EmailContext, EmailTemplateType, EmailServiceResponse } from './emailTemplateService';
...
class EmailNotificationService {
  ...
  private constructor() {
    this.emailService = EmailTemplateService.getInstance();
  ...
  private initializeRateLimiting(): void {
    ...
    try {
      ...
      console.warn('Failed to load rate limiting data:', error);
    }
  }
  ...
  private saveRateLimitData(): void {
    try {
      ...
      console.warn('Failed to save rate limiting data:', error);
    }
  }
  ...
  public async sendOrderShipped(orderData: OrderNotificationData & { 
    ...
    try {
      ...
      if (response.success) {
        console.log('✅ Order shipped notification sent for order:', orderData.orderNumber);
      }
  ...
  public async sendGDPRConsent(gdprData: GDPRNotificationData): Promise<EmailServiceResponse> {
    ...
    try {
      ...
      if (response.success) {
        console.log('✅ GDPR consent request sent to:', gdprData.userEmail);
      }

      return response;
    } catch (error) {
      console.error('Failed to send GDPR consent request:', error);
      return { success: false, error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }
